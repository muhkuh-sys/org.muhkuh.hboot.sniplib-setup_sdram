...
tElf = tEnv.Elf('targets/netx90_com_intram/setup_sdram_netx90_com_intram.elf', tSrc + tEnv['PLATFORM_LIBRARY'])
tTxt = tEnv.ObjDump('targets/netx90_com_intram/setup_sdram_netx90_com_intram.txt', tElf, OBJDUMP_FLAGS=['--disassemble', '--source', '--all-headers', '--wide'])
tBin = tEnv.ObjCopy('targets/netx90_com_intram/setup_sdram_netx90_com_intram.bin', tElf)
...
atSnippet = {
    ...
    'artifact': 'setup_sdram_netx90_mpw',
    ...
    'description': 'Setup the SDRAM on a netX90 MPW.',
    'categories': ['netx90MPW', 'booting', 'SDRAM'],
    'parameter': {
        'SDRAM_DATA_START': {'help': 'The start address of the SDRAM data area.'},
        'GENERAL_CTRL': {'help': 'The general control value.'},
        'TIMING_CTRL': {'help': 'The timing control value.'},
        'MODE_REGISTER': {'help': 'The mode register value.'}
    }
}
